        ...
        for dataset in datasets:
            ...
            for line in oldfile:
                ...
                if (len(splitLine) > 0) and (splitLine[0] == 'config.General.requestName'):
                    ...
                    checkFile.write("crab status -d crab_projects/crab_crab_clustAlg_QCD_%s_%s_%s\n"%(dataset,year,splitsubmitNumber))
                    resubmitFile.write("crab resubmit -d crab_projects/crab_crab_clustAlg_QCD_%s_%s_%s\n"%(dataset,year,splitsubmitNumber))
    checkFile.close()
    resubmitFile.close()
def main(argv): 
